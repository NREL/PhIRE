...
class SRGAN_surfRough(object):
    def __init__(self, x_LR=None, x_HR=None, x_rough = None, r=None, status='pre-training', loss_type='MSE'):
        ...
        if status in ['pre-training', 'training']:
            self.x_SR = self.generator(self.x_LR, self.x_rough, r=r, is_training=True)
            ...
            self.x_SR = self.generator(self.x_LR, self.x_rough, r=r, is_training=False)
    ...
    def generator(self, x, x_rough, r=None, is_training=False, reuse=False):
        if is_training:
            N, h, w, C = tf.shape(x)[0], x.get_shape()[1], x.get_shape()[2], x.get_shape()[3]
        else:
            N, h, w, C = tf.shape(x)[0], tf.shape(x)[1], tf.shape(x)[2], x.get_shape()[3]
        ...
        with tf.variable_scope('generator', reuse=reuse):
            ...
            # Update h and w for new resolution
            if is_training:
                _, h, w, _ = tf.shape(x)[0], x.get_shape()[1], x.get_shape()[2], x.get_shape()[3]
            else:
                _, h, w, _ = tf.shape(x)[0], tf.shape(x)[1], tf.shape(x)[2], x.get_shape()[3]

            #do something with roughness here.
            #append roughness onto x
            y = tf.concat([x, x_rough], 3)
            #add some layers for roughness processing

            for i in range(2):
                with tf.variable_scope('deconv_roughness{}'.format(i)):
                    y = deconv_layer_2d(y, [3, 3, C+1, C+1], [N, h, w, C+1], 1) # (x, filter_shape, output_shape, stride, trainable=True)
                    y = tf.nn.relu(y)

            with tf.variable_scope('deconv_out_final'):
                y = deconv_layer_2d(y, [3, 3, C_out, C+1], [N, h, w, C_out], 1)
